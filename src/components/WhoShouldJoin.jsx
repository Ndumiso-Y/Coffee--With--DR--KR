...
export default function WhoShouldJoin() {
  ...
  return (
    <section id="join" className="reveal py-20 bg-gradient-to-br from-cream via-white to-cream-50 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-10">
          {audiences.map((audience, index) => (
            <div key={audience.title} className="group relative" style={{animationDelay: `${index * 200}ms`}}>
              ...
              {/* Main card */}
              <div className="relative rounded-3xl p-10 bg-white/90 backdrop-blur-sm shadow-medium border border-white/50 group-hover:shadow-strong group-hover:-translate-y-3 transition-all duration-300 text-center">
                {/* Icon */}
                <div className={`inline-flex w-20 h-20 bg-gradient-to-r ${audience.gradient} rounded-3xl items-center justify-center mb-6 shadow-medium group-hover:scale-110 transition-transform duration-300`}>
                  <audience.icon className="w-10 h-10 text-white" />
                </div>

                {/* Content */}
                <h3 className="text-2xl font-bold text-deepCoffee mb-2 group-hover:text-coffeeOrange transition-colors duration-300">
                  {audience.title}
                </h3>
                <h4 className="text-xl font-semibold text-warmCoffee mb-4">
                  {audience.subtitle}
                </h4>
        ...
        <div className="text-center mt-16">
          ...
          <a href="#apply" className="inline-flex items-center rounded-2xl bg-orange-gradient px-8 py-4 text-white font-bold shadow-medium hover:shadow-glow transition-all duration-300 transform hover:-translate-y-1 hover:scale-105">
